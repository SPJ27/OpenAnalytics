...
const TRACKER_ID = "5d9da208-f3b2-40e5-9ab5-85423d08125e";
...
function getReferrerIcon(r) {
  ...
  if (/google/i.test(r))    return "🔍";
  if (/twitter|x\.com/i.test(r)) return "✖";
  if (/facebook/i.test(r))  return "📘";
  if (/github/i.test(r))    return "🐙";
  return "🌐";
}
function getDeviceIcon(d)   { return d === "Mobile" ? "📱" : d === "Tablet" ? "📟" : "🖥️"; }
      ...
      {options.map(o => (
        <button
          ...
          className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${
            active === o ? "bg-blue-500 text-white" : "text-slate-600 hover:text-slate-500"
          }`}
        >
          {o}
        </button>
          ...
          {PERIODS.map(p => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm transition-colors ${
                p === value ? "text-[#454545] bg-blue-600/10" : "text-[#303030] hover:bg-blue-600/5 hover:text-slate-600"
              }`}
            >
              {p}
            </button>
      ...
      <div className="max-w-5xl mx-auto space-y-4">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-1.5">
            ...
            <span className="text-sm font-light text-[#191919]">sakshamjain.dev</span>
          </div>
